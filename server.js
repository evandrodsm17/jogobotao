...
function calculateIdealBotPosition(bot, ball) {
  ...
  if (botRole === "DEFENDER") {
    ...
    idealX = isBotTeam1
      ? Math.min(idealX, maxDefensiveX)
      : Math.max(idealX, maxDefensiveX);
    ...
    if (isBallInOurHalf) {
      idealX = isBotTeam1
        ? Math.max(ball.x, homePos.x - 50)
        : Math.min(ball.x, homePos.x + 50);
      idealY = ball.y;
    }
    ...
    idealX = isBotTeam1
      ? Math.max(idealX, minOffensiveX)
      : Math.min(idealX, minOffensiveX);
